...
- 2026-06-20 XcodeBuildMCP focused simulator refresh passed `42/42` combined WeatherKit/WakeveAI tests on `iPhone 17`; the weather subset passed `15/15` selected tests. Artifacts: `test_sim_2026-06-20T19-56-28-754Z_pid9347_bb3368ef.log` and `test_sim_2026-06-20T19-56-28-754Z_pid9347_e5479204.xcresult`.
- `scripts/prepare-weatherkit-device-validation.sh` now generates the required real-device evidence template for 1.2/6.2 and is guarded by `scripts/test-critical-release-gates.sh`; it does not close either task until Apple Developer capability/profile and signed-device WeatherKit results are recorded.
- 2026-06-21 readiness refresh: `docs/weatherkit/weatherkit-device-validation-2026-06-20T23-08-57Z.md` still reports `PHYSICAL_IOS_DEVICE_NOT_TRACE_READY`; CoreDevice sees the physical iPhone, but Instruments lists it offline, `TEAM_ID`/`APPLE_TEAM_ID` is missing, there are `0` valid signing identities, and no provisioning profile matches `com.guyghost.wakeve`. The report now records Xcode 27.0 and iPhoneOS SDK 27.0 as the local toolchain baseline.
