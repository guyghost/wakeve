...
The system SHALL integrate with external meeting platforms through secure backend proxy APIs.
...
The system SHALL support the Zoom OAuth flow required for production meeting creation.
...
The system SHALL integrate with Google Meet through the Google Calendar API.
...
The system SHALL enforce per-provider rate limits for meeting platform APIs.
...
The system SHALL manage the full lifecycle of virtual meetings.
...
The system SHALL integrate virtual meetings with notification delivery.
...
The system SHALL synchronize virtual meetings with native device calendars.
...
#### Scenario: Sync meeting to native calendar
- **GIVEN** a virtual meeting is created or updated
- **WHEN** calendar integration is available
- **THEN** the system SHALL create or update the matching native calendar event
- **AND** cancellation SHALL remove or cancel the calendar event
