...
- `docs/APP_STORE_LIVE_URL_AASA_EVIDENCE.md` records that live DNS/AASA checks currently fail for `wakeve.app` legal/support/dashboard/AASA routes; `api.wakeve.app/health` is reachable, but backend smoke still cannot close until the uploaded TestFlight build proves the endpoint stays review-accessible during the monitoring window.
- `docs/ios-release-screen-evidence/release-screen-evidence-2026-06-13T13-32-47Z.md` records local simulator screenshot coverage for ROADMAP P1.2. It is useful preflight evidence only; event-detail and organization screenshots remain missing, the report names the next uploaded-build capture targets, and all rows must be revalidated from the uploaded TestFlight build before closure.
