...
Current result on 2026-09-03 local time: 7 live URL/AASA errors and 1 final-signoff warning remain expected for this blocker state. Both `wakeve.app` and `api.wakeve.app` now resolve publicly through Cloudflare; `https://wakeve.app/privacy`, `/support`, `/terms`, and `/third-party-notices` return HTTP 200, legacy landing redirects to `/app/*` validate, and `api.wakeve.app/health` is reachable (HTTP `405` for HEAD, HTTP `200 OK` for GET). The remaining errors are the five dashboard `/app` shell routes (HTTP 404 until the Cloudflare `wakeve-dashboard` worker routes are deployed) and both AASA endpoints (HTTP 503 until the production `APPLE_TEAM_ID`/`TEAM_ID` environment value is deployed). `docs/app-store-live-url-aasa/live-url-aasa-2026-09-03T20-38-08Z.md` records the matching command output. The previous baseline on 2026-06-21 local time recorded 17 live URL/AASA errors with `wakeve.app` DNS unresolved, captured in `docs/app-store-live-url-aasa/live-url-aasa-2026-06-20T22-48-00Z.md`. These errors keep AS-14 open until the dashboard `/app` routes and AASA files with the real Apple Team ID are live; `api.wakeve.app/health` must stay review-accessible through the final live smoke.
...
| AS-14 | Live URL/AASA validation | Public checks were rerun on 2026-09-03 local time: `wakeve.app` DNS/HTTPS and the legal/support/terms/third-party-notices pages are live, legacy redirects validate, and `api.wakeve.app/health` is reachable; the five dashboard `/app` routes return 404 (Cloudflare `wakeve-dashboard` worker not deployed) and both AASA endpoints return 503 pending the real Apple Team ID. | `https://wakeve.app/privacy`, `/terms`, `/support`, `/third-party-notices`, both AASA paths, and `https://api.wakeve.app/health` are reachable and validated with real `APPLE_TEAM_ID`; `docs/APP_STORE_LIVE_URL_AASA_EVIDENCE.md` contains `APP_STORE_LIVE_URL_AASA_EVIDENCE_COMPLETE=true` with deployment, DNS, command-output, cache, rollout, and rollback evidence. | `./scripts/app-store-submission-audit.sh --check-live-urls` |
