...
Current result on 2026-09-03 local time (after the Cloudflare web deploy): 2 live URL/AASA errors and 1 final-signoff warning remain expected for this blocker state. The Cloudflare Workers `wakeve-web` and `wakeve-dashboard` are deployed (`./scripts/deploy-cloudflare-web.sh`): legal pages return HTTP 200, the five dashboard `/app` routes are live (`/app` 308 to `/app/events`, others 200), legacy landing redirects to `/app/*` validate, and `api.wakeve.app/health` is reachable (HTTP `405` for HEAD, HTTP `200 OK` for GET). The only remaining errors are both AASA endpoints (HTTP 503 until the production `APPLE_TEAM_ID`/`TEAM_ID` environment value is deployed on the `wakeve-web` worker). `docs/app-store-live-url-aasa/live-url-aasa-2026-09-03T21-29-30Z.md` records the matching command output. Earlier baselines: 2026-09-03 pre-deploy (7 errors, capture `live-url-aasa-2026-09-03T20-38-08Z.md`) and 2026-06-21 (17 errors, DNS unresolved, capture `docs/app-store-live-url-aasa/live-url-aasa-2026-06-20T22-48-00Z.md`). These errors keep AS-14 open until the AASA files are served with the real Apple Team ID; `api.wakeve.app/health` must stay review-accessible through the final live smoke.
...
| AS-14 | Live URL/AASA validation | Public checks were rerun on 2026-09-03 after the Cloudflare web deploy: legal pages, the five dashboard `/app` routes, legacy redirects, and `api.wakeve.app/health` are live; both AASA endpoints return 503 pending the real Apple Team ID var on the `wakeve-web` worker. | `https://wakeve.app/privacy`, `/terms`, `/support`, `/third-party-notices`, both AASA paths, and `https://api.wakeve.app/health` are reachable and validated with real `APPLE_TEAM_ID`; `docs/APP_STORE_LIVE_URL_AASA_EVIDENCE.md` contains `APP_STORE_LIVE_URL_AASA_EVIDENCE_COMPLETE=true` with deployment, DNS, command-output, cache, rollout, and rollback evidence. | `./scripts/app-store-submission-audit.sh --check-live-urls` |
