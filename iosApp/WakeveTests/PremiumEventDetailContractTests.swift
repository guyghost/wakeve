...
final class PremiumEventDetailContractTests: XCTestCase {
    ...
    func testEventDetailWeatherCardUsesWeatherKitAndMapKit() throws {
        ...
        XCTAssertTrue(source.contains("resolveScenarioPlace(for event: Event)"))
        XCTAssertTrue(source.contains("ScenarioRepository(db: RepositoryProvider.shared.database)"))
        XCTAssertTrue(source.contains("getSelectedScenario(eventId: event.id)"))
        XCTAssertTrue(source.contains("getScenariosByEventId(eventId: event.id)"))
        XCTAssertTrue(source.contains("searchPlace(name: scenario.location, address: nil)"))
