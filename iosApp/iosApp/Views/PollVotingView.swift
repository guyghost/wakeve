...
#Preview("Poll Voting View") {
    ...
    let mockEvent = Event(
        ...
        proposedSlots: [
            TimeSlot(
                id: "slot-1",
                start: formatter.string(from: now.addingTimeInterval(86400)),
                end: formatter.string(from: now.addingTimeInterval(90000)),
                timezone: "Europe/Paris",
                timeOfDay: .morning
            ),
            TimeSlot(
                id: "slot-2",
                start: formatter.string(from: now.addingTimeInterval(172800)),
                end: formatter.string(from: now.addingTimeInterval(176400)),
                timezone: "Europe/Paris",
                timeOfDay: .afternoon
            )
        ],
        deadline: formatter.string(from: now.addingTimeInterval(604800)),
        status: .polling,
        finalDate: nil,
