...
struct BudgetDetailView: View {
    ...
    @State private var filteredItems: [BudgetItem] = []
    ...
    @State private var itemToEdit: BudgetItem?
    ...
    private var summaryHeader: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 20) {
                ...
                if budget.totalActualCost > budget.totalEstimatedCost {
                    ...
                    ForEach(filteredItems, id: \.id) { item in
                        BudgetItemCard(
                            ...
                            onEdit: {
                                itemToEdit = item
                                itemName = item.name
                                itemDescription = item.description_
    ...
    private var actualCostColor: Color {
        let percentage = budget.totalEstimatedCost > 0 
            ? (budget.totalActualCost / budget.totalEstimatedCost) * 100 
            : 0
    ...
    private func addItem() async {
        ...
        do {
            ...
            try await repository.createItem(item: newItem)
            resetForm()
            loadItems()
        } catch {
    ...
    private func updateItem() async {
        ...
        do {
            ...
            try await repository.updateItem(item: updatedItem)
            resetForm()
            loadItems()
        } catch {
    ...
    private func deleteItem(_ item: BudgetItem) async {
        do {
    ...
    private func markItemAsPaid(_ item: BudgetItem) async {
        do {
            let updatedItem = BudgetItem(
                id: item.id,
                budgetId: item.budgetId,
    ...
    static var previews: some View {
        BudgetDetailView(
            budget: Budget(
                ...
                totalActualCost: 850.0,
                estimatedTransportCost: 300.0,
                actualTransportCost: 280.0,
                estimatedAccommodationCost: 400.0,
                actualAccommodationCost: 380.0,
                estimatedMealsCost: 200.0,
                actualMealsCost: 150.0,
                estimatedActivitiesCost: 100.0,
                actualActivitiesCost: 40.0,
