...
- Le profil iOS expose maintenant des liens actifs vers support, privacy, terms et `https://wakeve.app/third-party-notices`; `FindingsRegressionTests.testProfileExposesRequiredLegalAndNoticeLinks` garde cette surface App Review.
...
- Les fichiers `iosApp/src/Resources/en.lproj/Localizable.strings` et `iosApp/src/Resources/fr.lproj/Localizable.strings` passent `plutil -lint`, ont chacun 829 cles et aucune cle manquante entre EN/FR apres localisation des labels d'accessibilite du login et des controles release supplementaires.
