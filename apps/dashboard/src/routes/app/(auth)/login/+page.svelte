<script lang="ts">
  ...
  const stateValue = $derived(auth.snapshot.value as string)
  const ctxError = $derived(auth.snapshot.context.error)
  const ctxEmail = $derived(auth.snapshot.context.email)
  ...
  function handleRequestOtp(e: SubmitEvent) {
    ...
    auth.actor.send({ type: 'REQUEST_OTP', email: email.trim() })
  }
  ...
  function handleVerifyOtp(e: SubmitEvent) {
    ...
    if (!trimmed) return
    auth.actor.send({ type: 'VERIFY_OTP', otp: trimmed })
  }

  function handleGuest() {
    auth.actor.send({ type: 'LOGIN_GUEST' })
  }

  function handleBack() {
    otp = ''
    auth.actor.send({ type: 'BACK' })
  }

  function handleResend() {
    auth.actor.send({ type: 'REQUEST_OTP', email: ctxEmail })
  }
</script>
